  ...
  return (
    ...
    >
      ...
      >
        ...
        <aside
          className={cn(
            "absolute right-0 top-0 h-full w-[82%] max-w-sm bg-cream-50 shadow-warm-lg",
            "flex flex-col gap-2 p-6 transition-transform duration-300",
        ...
        >
          <div className="flex items-center justify-between pb-6 border-b border-cream-300">
            ...
            <button
              ...
              className="inline-flex h-10 w-10 items-center justify-center rounded-full hover:bg-cream-200"
            >
              <X className="h-5 w-5" />
            </button>
          </div>
          <nav className="flex flex-col gap-1 pt-4">
            {navItems.map((item) => (
              <Link
                ...
                href={item.href}
                onClick={() => setOpen(false)}
                className="rounded-2xl px-4 py-3 text-base text-ink-soft transition-colors hover:bg-cream-200 hover:text-ink"
              >
                {item.label}
              </Link>
            ))}
          ...
          <Link
            ...
            rel="noopener noreferrer"
            onClick={() => setOpen(false)}
            className="mt-auto inline-flex h-12 items-center justify-center gap-2 rounded-full bg-terracotta-500 px-6 text-sm font-medium text-cream-50 shadow-soft hover:bg-terracotta-600"
          >
            Falar no WhatsApp
          </Link>
        </aside>
